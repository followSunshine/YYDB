...
#import "MyDBBModel.h"
@interface MyDBBViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MyDBBViewController
...
//    }
    ...
    self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        NSString *url = [NSString stringWithFormat:@"%@?ctl=uc_money&uid=%@&page=%d&v=3",urlpre,dele.userid,1];
        ...
        [manager GET:url parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            for (NSDictionary *dica in arr) {
                MyDBBModel *model = [[MyDBBModel alloc]initWithDictionary:dica error:nil];
...
- (void)setUI {
    ...
    NSString*str =[NSString stringWithFormat:@"%@币",del.totalMoney];
    NSMutableAttributedString *str1 = [[NSMutableAttributedString alloc]initWithString:str];
    [str1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:55] range:NSMakeRange(0, del.totalMoney.length)];
    [str1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:12] range:NSMakeRange(del.totalMoney.length, 1)];
    ...
    [self.tableView registerClass:[MyDBBTableViewCell class] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MyDBBTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
